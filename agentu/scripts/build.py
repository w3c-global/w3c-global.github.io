...
def build():
    ...
    files += [p for directory in (SITE / "demo", SITE / "brand") for p in directory.rglob("*") if p.is_file() and p.suffix in (".html", ".css", ".js", ".json", ".svg", ".txt")]
    for source in files:
    ...
    manifest = {"static_files": ["agentu/" + p.relative_to(SITE).as_posix() for p in files],
                "lambda_sha256": hashlib.sha256(package.read_bytes()).hexdigest(), "stage": "demonstration", "simulated": True}
